...
export function generatePreview(parameters, images, status, watchModelDescriptor) {
    ...
    for (const imageInfo of result) {
        ...
        const imageCountOffset = watchModelDescriptor.fileType.imageCountOffset || 0
        const zeroBasedImageId = imageInfo.imageId - imageCountOffset

        if (zeroBasedImageId < 0 || zeroBasedImageId >= images.length) {
            throw new Error(`Image Id out of range: ${imageInfo.imageId}. Valid range is ${imageCountOffset}-${images.length + imageCountOffset - 1}`)
        }
    }
